...
(Note that unlike R's C API, the code in this section also runs on Windows)
...
While it is preferred that C++ code is in its own `.cpp` file (and called using `sourceCpp("example.cpp")`), we have seen that C++ can be written inline from using the `code` argument of `sourceCpp()`. This inline code can be shortened using the `cppFunction()` function.
