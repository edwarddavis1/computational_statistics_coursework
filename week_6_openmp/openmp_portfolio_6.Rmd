...
Reduction is the process of combining or reducing the results of many sub-calculations into a single, reduced, result. Reduction requires the use of a critical section in order to keep track of some global result. For example, calculating iterations of a loop in parallel and using a critical section to update a global variable is an example of reduction. Generally, reduction is complicated to implement to be both work well and be efficient. To solve this, OpenMP provides a `reduction` directive, which has the following syntax.

```{cpp}
reduction( operator : variable list )
```

The following code, for example, will tell the compiler that `sum` will hold the global result of a reduction which will be formed by adding together the results of the thread-private calculations.
